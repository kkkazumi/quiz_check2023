...
int main(int argc, const char* argv[]){//(argv[0]=./estimate),argv[1]=usrname
  ...
  for(int set_num =0; set_num<SET_NUM_LEN; set_num++){
    printf("set num is %d\n",set_num);
...
void grad_descent(double M_org[N], double* M,MatrixXd W1_para,double (&factor)[N][8], double sig_total[1],int id_num,int moto_count, char* filename, char *dir_path,int set_num){
  ...
  double Msize[N];
  int count=0;
  char mental_log_second[100];
  sprintf(mental_log_second,"%soutput/mental_log_trial2%d-%d.csv",dir_path,N,set_num);
...
void get_para(double (&factor)[N][8], double (&signal)[N][1], double mental[N], MatrixXd *W1_para){
  ...
  MatrixXd Phi1_t(8,N);//,Phi2_t(10,N),Phi3_t(10,N),Phi4_t(10,N);//Phi共有できないのは、Mの値によって式の形が変わるため
  MatrixXd PP1(8,8);//,PP2(10,10),PP3(10,10),PP4(10,10);
  MatrixXd PP1_inv(8,8);//,PP2_inv(10,10),PP3_inv(10,10),PP4_inv(10,10);
